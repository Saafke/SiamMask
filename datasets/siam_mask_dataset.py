...
class SubDataSet(object):
        ...
        with open(cfg['anno']) as fin:
            ...
            for video in self.labels:
                for track in self.labels[video]:
                    ...
                    frames = list(map(int, filter(lambda x: isint(x),
                        frames.keys())))
                    frames.sort()
                    self.labels[video][track]['frames'] = frames
                    if len(frames) <= 0:
                        logger.info('warning {}/{} has no frames.'.format(
                            video, track))
            ...
            for trk, frames in tracks.items():
                ...
                for frm, bbox in frames.items():
                    ...
                    if len(bbox) == 4:
                        ...
                        w, h = bbox
                    if w == 0 or h == 0:
                        logger.info('Error, {name} {video} {trk} {bbox}'.
                            format(**locals()))
                        zero += 1
                        continue
                    new += 1
    ...
    def log(self):
        """I am a function."""
        logger.info(
            'SubDataSet {name} start-index {start} select [{select}/{num}] path {format}'
            .format(name=self.mark, start=self.start, select=self.num_use,
            num=self.num, format=self.path_format))
    ...
    def get_image_anno(self, video, track, frame):
        ...
        image_path = join(self.root, video, self.path_format.format(frame,
            track, 'x'))
        image_anno = self.labels[video][track][frame]
        mask_path = join(self.root, video, self.mask_format.format(frame,
            track))
...
def crop_hwc(image, bbox, out_sz, padding=(0, 0, 0)):
    ...
    crop = cv2.warpAffine(image, mapping, (out_sz, out_sz), borderMode=cv2.
        BORDER_CONSTANT, borderValue=padding)
    return crop
...
class Augmentation:
    ...
    def __init__(self, cfg):
        ...
        self.rgbVar = np.array([[-0.55919361, 0.98062831, -0.41940627], [
            1.72091413, 0.19879334, -1.82968581], [4.64467907, 4.73710203, 
            4.88324118]], dtype=np.float32)
        self.flip = 0
        self.eig_vec = np.array([[0.4009, 0.7192, -0.5675], [-0.814, -
            0.0045, -0.5808], [0.4203, -0.6948, -0.5836]], dtype=np.float32)
    ...
    def __call__(self, image, bbox, size, gray=False, mask=None):
        ...
        shape = image.shape
        crop_bbox = center2corner((shape[0] // 2, shape[1] // 2, size - 1, 
            size - 1))
        param = {}
        if self.shift:
            param['shift'] = Augmentation.random(
                ) * self.shift, Augmentation.random() * self.shift
        if self.scale:
            param['scale'] = 1.0 + Augmentation.random(
                ) * self.scale, 1.0 + Augmentation.random() * self.scale
        ...
        if self.scale:
            ...
            bbox = Corner(bbox.x1 / scale_x, bbox.y1 / scale_y, bbox.x2 /
                scale_x, bbox.y2 / scale_y)
        ...
        if self.resize:
            ...
            rand_size = int(round(ratio * imageSize[0])), int(round(ratio *
                imageSize[1]))
        ...
        if self.flip and self.flip > Augmentation.random():
            ...
            width = image.shape[1]
            bbox = Corner(width - 1 - bbox.x2, bbox.y1, width - 1 - bbox.x1,
                bbox.y2)
...
class AnchorTargetLayer:
    ...
    def __call__(self, anchor, target, size, neg=False, need_iou=False):
        ...
        x1, y1, x2, y2 = anchor_box[0], anchor_box[1], anchor_box[2
            ], anchor_box[3]
        cx, cy, w, h = anchor_center[0], anchor_center[1], anchor_center[2
            ], anchor_center[3]
        delta[0] = (tcx - cx) / w
        delta[1] = (tcy - cy) / h
        delta[2] = np.log(tw / w)
        ...
        cls[pos] = 1
        delta_weight[pos] = 1.0 / (pos_num + 1e-06)
        cls[neg] = 0
        if not need_iou:
            return cls, delta, delta_weight
        else:
...
class DataSets(Dataset):
    ...
    def __init__(self, cfg, anchor_cfg, num_epoch=1):
        ...
        self.anchors.generate_all_anchors(im_c=self.search_size // 2, size=
            self.size)
        ...
        self.num *= num_epoch
        self.shuffle()
        self.infos = {'template': self.template_size, 'search': self.
            search_size, 'template_small': self.template_small, 'gray':
            self.gray, 'neg': self.neg, 'inner_neg': self.inner_neg,
            'crop_size': self.crop_size, 'anchor_target': self.
            anchor_target.__dict__, 'num': self.num // num_epoch}
    ...
    def imread(self, path):
        ...
        def map_size(exe, size):
            """I am a function."""
            return int(round((exe + 1) / (self.origin_size + 1) * (size + 1
                ) - 1))
    ...
    def __getitem__(self, index, debug=False):
        ...
        template, _, _ = self.template_aug(template_image, template_box,
            self.template_size, gray=gray)
        search, bbox, mask = self.search_aug(search_image, search_box, self
            .search_size, gray=gray, mask=search_mask)
        ...
        if debug:
            draw(template_image, template_box, 'debug/{:06d}_ot.jpg'.format
                (index))
        ...
        if dataset.has_mask and not neg:
            ...
            mask_weight = np.zeros([1, cls.shape[1], cls.shape[2]], dtype=
                np.float32)
        template, search = map(lambda x: np.transpose(x, (2, 0, 1)).astype(
            np.float32), [template, search])
        mask = (np.expand_dims(mask, axis=0) > 0.5) * 2 - 1
        return template, search, cls, delta, delta_weight, np.array(bbox,
            np.float32), np.array(mask, np.float32), np.array(mask_weight,
            np.float32)
